...
const Tworalresultmajor = ({ uid }) => {
  const [userCourseId, setUserCourseId] = useState(null);
  const [loData, setLoData] = useState([]);
  const [majorproAverage, setmajorproAverage] = useState(0);
    ...
    const fetchCosData = async (uid) => {
      try {
        ...
        const [response1, response2, response3] = await Promise.all([
          api.get(`/api/result/majorpro/${uid}`),
          api.get(`/api/result/oral/${uid}`),
          api.get(`/api/result/indirect/${uid}`),
        ]);

        const majorproData = response1.data || [];
        ...
        const majorproMap = majorproData.reduce((acc, majorproItem) => {
          acc[majorproItem.coname] = Number(majorproItem.attainment) || 0;
          return acc;
        }, {});
        ...
        const combinedData = Array.from(
          new Set([...majorproData.map(item => item.coname), ...oralData.map(item => item.coname),...indirectData.map((item) => item.coname)])
        ).map((coname) => {
          const majorproattainment = majorproMap[coname] || 0;
          const oralattainment = oralMap[coname] || 0;
          const directAttainment = ((((60 / 100) * majorproattainment) + ((40 / 100) * oralattainment)) * (80 / 100)).toFixed(2);
          ...
          return {
            coname,
            majorproattainment: majorproattainment,
        ...
        const majorproAverage = combinedData
          .filter(item => item.majorproattainment)
          .reduce((sum, item) => sum + Number(item.majorproattainment), 0) / combinedData.length;
        ...
        const directAttainSixty = (60 / 100) * majorproAverage;
        ...
        setmajorproAverage(majorproAverage.toFixed(2));
  ...
  const downloadExcel = () => {
    ...
    const headers = [
      { coname: 'CO/LO', majorproattainment: 'TW', oralattainment: 'OR', conameIndirect: 'Indirect CO/LO', indirect: 'Indirect', direct: 'Direct Attainment', indirectatt: 'Indirect Attainment', total: 'Total Attainment' }
    ];
    ...
    const loDataForExport = [
      ...
      ...loData.map((item) => ({
        coname: item.coname,
        majorproattainment: item.majorproattainment,
    ...
    const additionalData = [
      { coname: "Attainment", majorproattainment: majorproAverage, oralattainment: oralAverage, conameIndirect: "Final Indirect Course Attainment", indirect: FinalIndirectCourseAttainment },
      { coname: "Weightage", majorproattainment: "60%", oralattainment: "40%" },
      { coname: "Direct Total Attainment", majorproattainment: DirectTotalAttainSixty, oralattainment: DirectTotalAttainForty },
      { coname: "Final Direct Course Attainment", majorproattainment: FinalDirectCourseAttainment, oralattainment: "", conameIndirect: "", indirect: "" },
      { coname: "Weightage", majorproattainment: "80%", oralattainment: "", conameIndirect: "20%" },
      { coname: "Total Attainment", majorproattainment: TotalAttainmentEighty, oralattainment: "", conameIndirect: TotalAttainmentTwenty },
      { coname: "Course Attainment", majorproattainment: TotalAttainment, oralattainment: "", conameIndirect: "", indirect: "" }
    ];
    ...
    XLSX.writeFile(workbook, "TW_ORAL_MajorPro_Attainment.xlsx");
  };
          ...
          <tbody>
            ...
            {loData.map((item, index) => (
              <tr key={index}>
                ...
                <td className="border border-gray-300 p-2 text-center">{item.majorproattainment}</td>
            ...
            <tr>
              ...
              <td className="border border-gray-300 p-2 text-center">{majorproAverage}</td>
...
export default Tworalresultmajor;
