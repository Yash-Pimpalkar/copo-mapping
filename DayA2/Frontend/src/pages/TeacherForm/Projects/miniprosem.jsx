...
//     }));
//   };
const handleMarksChange = (event, index, field) => {
    const value = event.target.value;
  
    if (value === "" || value < 0 || value > maxLimit) {
      alert(`Value should be between 0 and ${maxLimit}`);
      return;
    }
  ...
  function calculateCoAverages(userCourse, MiniproData, maxLimit, attainmentData) {
    return userCourse.map((course) => {
      ...
      const totalStudents = MiniproData.length;
  ...
  const handle_Attainment = (userCourse, MiniproData, maxLimit, attainmentData, userCourseId) => {
    if (userCourseId) {
      // Calculate CO averages
      const coAverages = calculateCoAverages(userCourse, MiniproData, maxLimit, attainmentData);
