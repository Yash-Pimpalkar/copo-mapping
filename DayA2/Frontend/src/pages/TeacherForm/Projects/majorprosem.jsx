...
const MajorproSem = ({ uid }) => {
    ...
    const [MajorproData, SetMajorproData] = useState([]);
    ...
    const [attainmentData, setAttainmentData] = useState({
        passedPercentage: 50, // Default to 50%
    });
    useEffect(() => {
        ...
        const fetchMajorproData = async () => {
            if (userCourseId) {
                try {
                    ...
                    // Fetching the limits
                    const res2 = await api.get(`/api/uploadmajorprosem/limit/${userCourseId}`);
    ...
    const studentsBelowHalf = MajorproData.filter(
        (student) =>
    ...
    const passedPercentage = (countOfStudentsBelowHalf / MajorproData.length) * 100;
    ...
    const calculateTotal = (student) => {
        const logbookmarks = parseFloat(student.logbookmarks) || 0;
        const review1marks = parseFloat(student.review1marks) || 0;
        const review2marks = parseFloat(student.review2marks) || 0;
        const proreportmarks = parseFloat(student.proreportmarks) || 0;

        // Calculate average of review1_marks and review2_marks
        const avgReviews = (review1marks + review2marks) / 2;

        // Calculate total marks
        const totalMarks = logbookmarks + avgReviews + proreportmarks;
    ...
    const totalItems = MajorproData.length;
    console.log("Major:", MajorproData);
    console.log("Total count of MajorproData:", MajorproData.length);
    const totalPages = Math.ceil(totalItems / itemsPerPage);
    const startIndex = (currentPage - 1) * itemsPerPage;
    // console.log(startIndex);
    const endIndex = startIndex + itemsPerPage;
    const paginatedData = MajorproData.slice(startIndex, endIndex);

    const filteredData = MajorproData.filter((item) => {
    ...
    const handleEditClick = (index) => {
        ...
        setEditedMarks({
            ...editedMarks,
            [index]: {
                logbook_marks: MajorproData[index].logbook_marks,
                review1_marks: MajorproData[index].review1_marks,
                review2_marks: MajorproData[index].review2_marks,
                proreportmarks: MajorproData[index].project_report_quality,
                avg_reviews: MajorproData[index].avg_reviews,
            },
        });
    };
   ...
   console.log(MajorproData)
    ...
    const logbookmarks = MajorproData.map(item => item.logbookmarks);
    const review1marks = MajorproData.map(item => item.review1marks);
    const review2marks = MajorproData.map(item => item.review2marks);
    const projectreportquality = MajorproData.map(item => item.proreportmarks);
    ...
    const filteredDataAttainment = MajorproData.filter(item => {
        let isValid = true;
    ...
    const avglogbookattainment = ((validLogbookmarksCount / MajorproData.length) * 100).toFixed(2);
    const avgreviews1attainment = ((validReview1marksCount / MajorproData.length) * 100).toFixed(2);
    const avgreviews2attainment = ((validReview2marksCount / MajorproData.length) * 100).toFixed(2);
    const avgprereportattainment = ((validProreportmarksCount / MajorproData.length) * 100).toFixed(2);
    ...
    const handleFileDownload = () => {
        const formattedData = MajorproData.map((student) => {
            ...
            return {
                mainmajorprosemid: student.mainmajorprosemid,
        ...
        const headers = ["Majorpro Id", "Student ID", "Student Name", "Logbook Marks", "Review1 Marks", "Review2 Marks", "Avg of Reviews", "Project Report Quality", "Total"];
        const dataWithHeaders = [headers, ...formattedData.map(Object.values)];

        const worksheet = XLSX.utils.aoa_to_sheet(dataWithHeaders);
        const workbook = XLSX.utils.book_new();
        XLSX.utils.book_append_sheet(workbook, worksheet, "Majorproject Data");
        XLSX.writeFile(workbook, "Majorproject data.xlsx");
    };
    ...
    const handle_Attainment = (userCourse, MajorproData, maxLimitlogbook, maxLimitreview1, maxLimitreview2, maxLimitproreport, attainmentData, userCourseId) => {
        if (userCourseId) {
            ...
            api
                .post("/api/uploadmajorprosem/insert-co-averages", {
                    ...
                    <div className="container mx-auto bg-white shadow-lg rounded-lg p-6 mt-6">
                        ...
                        <div className="mt-4">
                            <h2 className="text-md font-semibold">
                                Total Students with Marks Below 50%: {countOfStudentsBelowHalf} / {MajorproData.length}
                            </h2>
                            <p>
                            ...
                            <table>
                                ...
                                <tbody>
                                    ...
                                    <tr>
                                        ...
                                        <td>
                                            {(MajorproData.length + MajorproData.length + MajorproData.length + MajorproData.length) / 4}
                                        </td>
                                    </tr>
                                    <tr>
                                        <td>
