        ...
        const fetchFeedbackData = async () => {
            if (userCourseId) {
                ...
                try {
                    ...
                    const res = await api.get(`/api/lmsclassroom/feedback/show/${userCourseId}`);
            ...
            <div className="p-4 sm:p-8 lg:p-12 bg-gray-100 min-h-screen flex flex-col items-center bg-gradient-to-r from-blue-50 to-blue-100">
                ...
                <div className="bg-white p-4 sm:p-6 rounded-lg shadow-md w-full max-w-8xl">
                    ...
                    <div className="flex flex-wrap justify-between sm:justify-start space-y-2 sm:space-y-0 space-x-0 sm:space-x-4 mt-4 border-b border-gray-200">
                        <button className="py-2 px-4 w-full sm:w-auto border-b-0 border-blue-500 text-gray-600 font-medium hover:text-blue-500 font-medium">Feedback</button>
                        ...
                        <button onClick={handleAnalysisClick} className="py-2 px-4 w-full sm:w-auto text-gray-600 font-medium hover:text-blue-500 font-medium">Analysis</button>
                        <button onClick={handleResponseClick} className="py-2 px-4 w-full sm:w-auto text-gray-600 font-medium hover:text-blue-500 font-medium">Responses</button>
                    </div>
                    ...
                    {userCourseId && !showAnalysis && !showResponseFeedback && FeedbackData && FeedbackData.length > 0 && (
                        <>
                            {/* Feedback Box */}
                            <div className="mt-6 bg-gray-50 p-4 rounded-lg border border-gray-200 shadow-sm">
                                <p className="mt-4 text-gray-700">
                                    <span className="font-semibold">Opened:</span> {FeedbackData[0].created_at ? formatDate(FeedbackData[0].created_at) : 'N/A'}<br />
                                    <span className="font-semibold">Closes:</span> {FeedbackData[0].deadline ? formatDate(FeedbackData[0].deadline) : 'N/A'}
                                </p>
                            </div>
