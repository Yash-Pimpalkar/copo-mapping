...
export default function Cos_reg({ id }) {
    const [isSubmitted, setIsSubmitted] = useState(false);
    const [numCos, setNumCos] = useState(1); // Number of COS records user wants to add
    const [formData, setFormData] = useState([{ cos_id: "", cos_name: "", cos_body: "" }]); // Initial state for form data
    ...
    const handleNumCosChange = (e) => {
        ...
        setNumCos(value);
        const updatedFormData = Array.from({ length: value }, (_, index) => formData[index] || { cos_id: "", cos_name: "", cos_body: "" });
        setFormData(updatedFormData);
    };

    // Validate all dynamically generated fields
    const handleValidation = () => {
        const requiredFields = ["cos_id", "cos_name", "cos_body"];
