...
export default function Course_reg({ id }) {
    ...
    const [numCourses, setNumCourses] = useState(1); // Number of courses user wants to add
    const [formData, setFormData] = useState([{ course_id: "", course_code: "", course_name: "" }]); // Initial state for form data
    ...
    // Handle change in the number of courses input
    const handleNumCoursesChange = (e) => {
        const value = e.target.value;
        setNumCourses(value);
        const updatedFormData = Array.from({ length: value }, (_, index) => formData[index] || { course_id: "", course_code: "", course_name: "" });
        setFormData(updatedFormData);
    };

    // Validate all dynamically generated fields
    const handleValidation = () => {
        const requiredFields = ["course_id", "course_code", "course_name"];
